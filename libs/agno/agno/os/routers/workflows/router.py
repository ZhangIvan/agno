        ...
        async def _drive_continue_stream():
            try:
                ...
                async for event in response_stream:
                    event_dict = event.model_dump() if hasattr(event, "model_dump") else event.to_dict()
                    await websocket.send_text(json.dumps(event_dict, default=json_serializer, ensure_ascii=False))
            except Exception as e:
        ...
        try:
            while True:
                ...
                # Handle authenticated actions
                    ...
                    # Build auth context for ownership verification
                    from agno.os.scopes import AgentOSScope

                    ws_auth = WebSocketAuthContext(
                        jwt_enabled=jwt_auth_enabled,
                        is_admin=bool(
                            websocket_user_context
                            and websocket_user_context.get("scopes")
                            and AgentOSScope.ADMIN.value in websocket_user_context["scopes"]
                        ),
                    )
                    # Add user context to message if available from JWT auth
                    if websocket_user_context:
