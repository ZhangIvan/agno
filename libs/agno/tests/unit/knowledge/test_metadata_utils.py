...
class TestMergeUserMetadata:
    ...
    def test_agno_keys_are_deep_merged(self):
        existing = {RESERVED_AGNO_KEY: {"source_type": "s3", "bucket": "my-bucket"}, "user_key": "x"}
        incoming = {RESERVED_AGNO_KEY: {"source_url": "https://example.com"}, "user_key": "y"}
        result = merge_user_metadata(existing, incoming)
        assert result[RESERVED_AGNO_KEY] == {
            "source_type": "s3",
            "bucket": "my-bucket",
    ...
    def test_agno_incoming_overwrites_conflicting_agno_keys(self):
        existing = {RESERVED_AGNO_KEY: {"status": "old"}}
        incoming = {RESERVED_AGNO_KEY: {"status": "new"}}
        result = merge_user_metadata(existing, incoming)
        assert result[RESERVED_AGNO_KEY]["status"] == "new"

    def test_agno_non_dict_incoming_treated_as_empty(self):
        existing = {RESERVED_AGNO_KEY: {"key": "value"}}
        incoming = {RESERVED_AGNO_KEY: "not-a-dict"}
        result = merge_user_metadata(existing, incoming)
        assert result[RESERVED_AGNO_KEY] == {"key": "value"}

    def test_does_not_mutate_existing(self):
        existing = {"a": 1, RESERVED_AGNO_KEY: {"x": 1}}
        incoming = {"a": 2, RESERVED_AGNO_KEY: {"y": 2}}
        original_existing = {"a": 1, RESERVED_AGNO_KEY: {"x": 1}}
...
class TestSetAgnoMetadata:
    def test_sets_key_on_none_metadata(self):
        ...
        assert result == {RESERVED_AGNO_KEY: {"source_type": "url"}}

    def test_sets_key_on_empty_metadata(self):
        result = set_agno_metadata({}, "source_type", "s3")
        assert result == {RESERVED_AGNO_KEY: {"source_type": "s3"}}
    ...
    def test_sets_key_preserving_existing_user_metadata(self):
        ...
        assert result[RESERVED_AGNO_KEY]["source_type"] == "gcs"

    def test_sets_key_preserving_existing_agno_keys(self):
        metadata = {RESERVED_AGNO_KEY: {"existing_key": "keep"}}
        result = set_agno_metadata(metadata, "new_key", "added")
        assert result[RESERVED_AGNO_KEY] == {"existing_key": "keep", "new_key": "added"}

    def test_overwrites_existing_agno_key(self):
        metadata = {RESERVED_AGNO_KEY: {"status": "old"}}
    ...
    def test_handles_none_agno_value_in_metadata(self):
        metadata = {RESERVED_AGNO_KEY: None}
...
class TestGetAgnoMetadata:
    ...
    def test_returns_none_when_agno_is_not_dict(self):
        assert get_agno_metadata({RESERVED_AGNO_KEY: "string"}, "key") is None

    def test_returns_none_when_key_not_in_agno(self):
        assert get_agno_metadata({RESERVED_AGNO_KEY: {"other": "value"}}, "key") is None

    def test_returns_value_for_existing_key(self):
        metadata = {RESERVED_AGNO_KEY: {"source_type": "url", "source_url": "https://example.com"}}
    ...
    def test_returns_falsy_values_correctly(self):
        metadata = {RESERVED_AGNO_KEY: {"count": 0, "flag": False, "empty": ""}}
...
class TestStripAgnoMetadata:
    ...
    def test_strips_agno_key(self):
        metadata = {"user_key": "value", RESERVED_AGNO_KEY: {"source_type": "s3"}}
        result = strip_agno_metadata(metadata)
        assert result == {"user_key": "value"}
        assert RESERVED_AGNO_KEY not in result

    def test_returns_copy_not_original(self):
        metadata = {"key": "value", RESERVED_AGNO_KEY: {"x": 1}}
        result = strip_agno_metadata(metadata)
        assert result is not metadata

    def test_preserves_all_non_agno_keys(self):
        metadata = {"a": 1, "b": "two", "c": [3], RESERVED_AGNO_KEY: {"internal": True}}
        result = strip_agno_metadata(metadata)
    ...
    def test_does_not_mutate_original(self):
        ...
        assert RESERVED_AGNO_KEY in metadata
...
def test_reserved_agno_key_value():
    assert RESERVED_AGNO_KEY == "_ag_os"
